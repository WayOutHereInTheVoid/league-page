...
export async function load() {
  try {
    ...
    console.log(
      `Current league: ${currentLeagueData.season} (${currentLeagueData.status})`,
    );
    ...
    if (
      currentLeagueData.status === "pre_draft" &&
      currentLeagueData.previous_league_id
    ) {
      ...
      try {
        ...
        console.log(
          `Previous league: ${prevLeagueData.season} (${prevLeagueData.status})`,
        );
        ...
        if (prevLeagueData.status === "complete") {
          ...
          if (
            ...
            result.standingsData = {
              standingsInfo: standings,
              yearData: prevLeagueData.season,
            };
          ...
          console.log(
            `❌ Previous season status: ${prevLeagueData.status} (not complete)`,
          );
        }
      } catch (error) {
