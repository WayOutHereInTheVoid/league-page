...
function validateData(context, data, expectedFields = []) {
    ...
    if (!data) {
        debugLog(`${context} - ERROR`, { error: 'Data is null or undefined' }, 'error');
        return false;
    }
...
function processSeasonDataDebug(yearData, awards, managerRosters) {
    ...
    if (awards && Array.isArray(awards)) {
                ...
                if (award.divisions && Array.isArray(award.divisions)) {
                    for (const division of award.divisions) {
                        if (division.rosterID === rosterNum) {
                            playoffs = true;
                            divisionChamp = true;
                            debugLog('Division Championship Found', { rosterNum, division, year: award.year });
                        }
                    }
                }
            }
        ...
        debugLog('Awards Processing - WARNING', { 
    ...
    if (yearData.fptsAgainst !== undefined && yearData.fptsAgainst !== null) {
        ...
        // Handle decimal separately if it exists
        const decimal = yearData.fpts_against_decimal ? yearData.fpts_against_decimal / 100 : 0;
        fptsAgainst = parseFloat(yearData.fpts_against) + decimal;
    } else {
        debugLog('Points Against - ERROR', { 
    ...
    if (yearData.potentialPoints !== undefined && yearData.potentialPoints !== null) {
        ...
        debugLog('Potential Points - WARNING', { 
    ...
    const season = {
        ...
        lineupEfficiency: finalPotentialPoints > 0 ? 
            parseFloat(round((finalFpts / finalPotentialPoints) * 100)) : 0,
 ...
 */
export function computeManagerStatsDebug(manager, leagueTeamManagers, records, currentRosterID, awards) {
    debugLog('computeManagerStats - Start', {
        manager: manager ? { name: manager.name, managerID: manager.managerID } : 'null',
    ...
    if (!manager?.managerID) {
        debugLog('Manager Validation - WARNING', { 
    ...
    const managerRosters = getAllManagerRosterIDsDebug(manager.managerID, leagueTeamManagers);
    ...
    if (Object.keys(managerRosters).length === 0) {
        debugLog('Manager Rosters - ERROR', { 
...
function getAllManagerRosterIDsDebug(managerID, leagueTeamManagers) {
    ...
    if (!leagueTeamManagers?.teamManagersMap) {
        debugLog('getAllManagerRosterIDs - ERROR', { 
...
function aggregateManagerSeasonDataDebug(managerRosters, records) {
    ...
    if (!records?.regularSeasonData?.leagueRosterRecords) {
        debugLog('aggregateManagerSeasonData - ERROR', { 
