...
export const analyzeTransactionComplexity = (transaction) => {
    if (transaction.type === 'trade') {
        ...
        const estimatedValue = totalPlayers * 2 + (hasDraftPicks ? 5 : 0) + (hasFAAB ? 3 : 0);
    ...
    if (transaction.type === 'waiver') {
        const hasBid = transaction.moves[0] && transaction.moves[0][0] && transaction.moves[0][0].bid;
...
export const calculateTransactionImpact = (transaction) => {
    ...
    if (transaction.type === 'trade') {
        ...
        return {
            ...
            visualWeight: complexity === 'high' ? 4 : complexity === 'medium' ? 2.5 : 1
        };
    }
...
export const generateGridPerformanceInsights = (transactions) => {
    ...
    return {
        ...
        columnDistribution,
        balanceScore: Math.abs((columnDistribution[1] || 0) - (columnDistribution[2] || 0)),
        recommendations: {
            useGrid: transactions.length >= 4,
            balanceQuality: Math.abs((columnDistribution[1] || 0) - (columnDistribution[2] || 0)) <= 2 ? 'good' : 'needs_improvement'
        }
    };
};
