  ...
  try {
    ...
    if (enableEnhancedStats) {
      try {
        ...
        usedEnhanced = true;
        debugAwardProcessing("Enhanced stats computation successful", {
          managerID: manager?.managerID,
        });
      } catch (enhancedError) {
        debugAwardProcessing(
          "Enhanced stats failed, falling back to standard",
          {
            managerID: manager?.managerID,
    ...
    if (enableValidation && managerStats) {
      ...
      if (!validationResult.isValid && !fallbackOnError) {
        throw new Error(
          `Manager statistics validation failed: ${validationResult.errors.map((e) => e.message).join(", ")}`,
        );
      }
    ...
    debugAwardProcessing(
      "Manager stats computation failed",
      {
        managerID: manager?.managerID,
        error: error.message,
      },
  ...
  validationResult.errors.forEach((error) => {
    switch (error.type) {
      ...
      case "out_of_range":
        ...
        if (
          error.field === "winPercentage" ||
          error.field === "win_percentage"
        ) {
  ...
  function fixObject(obj) {
    ...
    Object.keys(obj).forEach((key) => {
      ...
      if (typeof value === "number" && isNaN(value)) {
        ...
        debugAwardProcessing(
          "Fixed NaN value",
          { field: key, oldValue: value, newValue: obj[key] },
          "warn",
        );
  ...
  for (let i = 0; i < managers.length; i += maxConcurrent) {
      ...
      try {
        ...
        if (managerResult.validationResult) {
          results.summary.warnings +=
            managerResult.validationResult.warnings?.length || 0;
          results.summary.errors +=
            managerResult.validationResult.errors?.length || 0;
        }

        return managerResult;
      } catch (error) {
        debugAwardProcessing(
          "Manager processing failed",
          {
            managerID: manager?.managerID,
            error: error.message,
          },
    ...
    if (progressCallback) {
      progressCallback({
        ...
        total: managers.length,
        percentage: Math.min(
          ((i + maxConcurrent) / managers.length) * 100,
          100,
        ),
      });
    }
  ...
  const preparedStats = {
    ...
    _validationSummary: validatedManagerResult.validationResult
      ? {
          ...
          errorCount:
            validatedManagerResult.validationResult.errors?.length || 0,
          warningCount:
            validatedManagerResult.validationResult.warnings?.length || 0,
        }
      : null,
  };
