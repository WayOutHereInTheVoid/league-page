...
export function validateAndNormalizeAwards(awards) {
  ...
  for (const award of awards) {
    try {
      ...
      console.error(
        "[Awards Validation] Error validating award:",
        error,
        award,
      );
...
function validateSingleAward(award) {
  ...
  if (
    !validatedAward.champion &&
    !validatedAward.second &&
    !validatedAward.third &&
...
export function detectAwardsForRoster(rosterID, year, validatedAwards) {
  ...
  const yearAwards = validatedAwards.filter(
    (award) => award.year === normalizedYear,
  );
  ...
  detectionLog.push(
    `Found ${yearAwards.length} award(s) for year ${normalizedYear}`,
  );
  ...
  for (const award of yearAwards) {
    ...
    if (award.champion === normalizedRosterID) {
      ...
      playoffs = true;
      detectionLog.push(
        `Championship detected for roster ${normalizedRosterID}`,
      );
    }
    ...
    if (award.second === normalizedRosterID) {
      ...
      playoffs = true;
      detectionLog.push(
        `Runner-up (2nd place) detected for roster ${normalizedRosterID}`,
      );
    }
    ...
    if (award.third === normalizedRosterID) {
      ...
      playoffs = true;
      detectionLog.push(
        `Third place detected for roster ${normalizedRosterID}`,
      );
    }
    ...
    if (award.toilet === normalizedRosterID) {
      toilet = true;
      detectionLog.push(
        `Toilet bowl detected for roster ${normalizedRosterID}`,
      );
    }

    // Division championship check
    if (award.divisions && award.divisions.length > 0) {
      const divisionWin = award.divisions.find(
        (div) => div.rosterID === normalizedRosterID,
      );
      if (divisionWin) {
        divisionChamp = true;
        playoffs = true;
        detectionLog.push(
          `Division championship detected for roster ${normalizedRosterID} (${divisionWin.name || "Unknown Division"})`,
        );
      }
    }
  }
...
export function generateAwardsValidationReport(rawAwards) {
  ...
  for (const award of validatedAwards) {
    ...
    if (award.divisions)
      report.statistics.divisionChampionships += award.divisions.length;
  }
  ...
  const missingChampions = validatedAwards.filter(
    (award) => !award.champion,
  ).length;
