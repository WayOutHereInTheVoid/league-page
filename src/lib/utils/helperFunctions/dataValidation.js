    ...
    if (typeof leagueData.settings !== "object") {
      ...
      if ("playoff_week_start" in leagueData.settings) {
        ...
        if (typeof playoffWeek === "number") {
          const playoffValidation = validateRange(
            playoffWeek,
            13,
            18,
            "playoff_week_start",
...
export function validateManagerStats(managerStats, managerID = null) {
  ...
  if ("seasons" in managerStats) {
    if (!Array.isArray(managerStats.seasons)) {
      ...
      managerStats.seasons.forEach((season, index) => {
        const seasonValidation = validateSeasonData(
          season,
  ...
  if ("totalStats" in managerStats) {
    const totalStatsValidation = validateTotalStats(
...
export function validateSeasonData(seasonData, context = "season") {
  ...
  if (typeof seasonData.year === "number") {
    const currentYear = new Date().getFullYear();
    const yearValidation = validateRange(
      seasonData.year,
      2009,
      currentYear + 1,
      "year",
  ...
  const totalGames =
    (seasonData.wins || 0) + (seasonData.losses || 0) + (seasonData.ties || 0);
  if (totalGames > 0) {
  ...
  if (typeof seasonData.fpts === "number") {
    const avgPointsPerGame =
      totalGames > 0 ? seasonData.fpts / totalGames : seasonData.fpts;
  ...
  if (
    "lineupEfficiency" in seasonData &&
    typeof seasonData.lineupEfficiency === "number"
  ) {
    const efficiencyValidation = validateRange(
      seasonData.lineupEfficiency,
      0,
      100,
      "lineup efficiency",
    );
...
export function validateMatchupConsistency(matchupsData, expectedRosterIds) {
  ...
  if (!Array.isArray(matchupsData) || !Array.isArray(expectedRosterIds)) {
    result.addError(
      ...
      null,
      {
        matchupsType: typeof matchupsData,
        rostersType: typeof expectedRosterIds,
      },
    );
    return result;
  }
  ...
  matchupsData.forEach((weekMatchups, weekIndex) => {
    ...
    const missingRosters = expectedRosterIds.filter(
      (id) => !weekRosterIds.includes(id),
    );
    const extraRosters = weekRosterIds.filter(
      (id) => !expectedRosterIds.includes(id),
    );
...
export function validateJsonStructure(data, expectedStructure = null) {
  ...
  try {
    ...
    if (expectedStructure && typeof expectedStructure === "object") {
      const structureValidation = validateRequiredFields(
        data,
...
export function logValidationResult(
  result,
  context = "validation",
  level = VALIDATION_CONFIG.VALIDATION_LEVELS.WARN,
) {
  ...
  if (!result.isValid) {
    ...
    level !== VALIDATION_CONFIG.VALIDATION_LEVELS.SILENT
  ) {
    ...
    try {
      ...
      if (validator) {
        ...
        if (!validation.isValid) {
          throw new Error(
            `Validation failed for ${context}: ${validation.errors.map((e) => e.message).join(", ")}`,
          );
        }
      ...
      console.error(
        `[${context.toUpperCase()}] Function execution failed:`,
        error,
      );
      throw error;
    }
