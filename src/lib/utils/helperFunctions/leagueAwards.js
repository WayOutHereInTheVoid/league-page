...
const getPodiums = async (previousSeasonID) => {
	...
	while(previousSeasonID && previousSeasonID != 0) {
		try {
			...
			const previousSeasonData = await getPreviousLeagueDataEnhanced(previousSeasonID);

			if (!previousSeasonData) {
				debugAwardProcessing('Season Data Retrieval Failed', {
			...
			if (!championshipInfo.champion) {
				debugAwardProcessing(`Season ${year} - Skipping`, {
			...
			const podium = {
				...
				detectionMethods: {
					...
					confidence: calculatePodiumConfidence(championshipInfo, toiletInfo, playoffStructure)
				}
			};
			...
			debugAwardProcessing(`Season ${year} - Podium Created`, {
				podium: {
					...podium,
					playoffParticipants: podium.playoffParticipants.length + ' participants'
				}
			});
			...
			debugAwardProcessing(`Season Processing Error`, {
				seasonID: previousSeasonID,
				error: error.message
			}, 'error');
			...
			try {
				...
				debugAwardProcessing('Fallback Failed - Stopping', {
					seasonID: previousSeasonID,
...
const getPreviousLeagueDataEnhanced = async (previousSeasonID) => {
	try {
		...
		const [leagueRes, rostersData, losersRes, winnersRes] = await waitForAll(...resPromises);
		...
		try {
			...
			debugAwardProcessing('Playoff Rounds Extraction Failed', {
				seasonID: previousSeasonID,
				error: error.message
			}, 'warn');
		}
		...
		debugAwardProcessing('Enhanced League Data Fetch Failed', {
			seasonID: previousSeasonID,
			error: error.message
		}, 'error');
		return null;
 ...
 */
function extractChampionshipInfoEnhanced(winnersData, losersData, playoffStructure, playoffRounds) {
	...
	if (playoffStructure.championship?.championshipMatch) {
		const { champion, runnerUp } = playoffStructure.championship.championshipMatch;
		...
		if (playoffStructure.championship.thirdPlaceMatch) {
			championshipInfo.third = playoffStructure.championship.thirdPlaceMatch.thirdPlace;
		}

		debugAwardProcessing('Championship - Playoff Structure Method', championshipInfo);
		return championshipInfo;
	}

	// Method 2: Legacy detection (original logic as fallback)
	try {
		const finalsMatch = winnersData.filter(m => 
			m.r == playoffRounds && m.t1_from?.w
		)[0];
		...
		if (finalsMatch && finalsMatch.w) {
			...
			const runnersUpMatch = winnersData.filter(m => 
				m.r == playoffRounds && m.t1_from?.l
			)[0];
		...
		debugAwardProcessing('Championship - Legacy Method Failed', {
			error: error.message
		}, 'warn');
	}
	...
	try {
		...
		if (winnerMatch) {
			...
			debugAwardProcessing('Championship - Final Round Method', championshipInfo);
		}
	} catch (error) {
		debugAwardProcessing('Championship - Final Round Method Failed', {
			error: error.message
		}, 'warn');
	}
 ...
 */
function extractToiletBowlInfoEnhanced(losersData, toiletRounds, playoffStructure) {
	let toiletInfo = {
		toilet: null,
		detectionMethod: 'none'
	};
	...
	try {
		const toiletBowlMatch = losersData.filter(m => 
			m.r == toiletRounds && (!m.t1_from || m.t1_from.w)
		)[0];
		...
		debugAwardProcessing('Toilet Bowl - Original Method Failed', {
			error: error.message
		}, 'warn');
	}
	...
	try {
		...
		const finalLoserMatch = losersData.find(m => 
			m.r === finalLoserRound && m.w
		);
		...
		debugAwardProcessing('Toilet Bowl - Final Round Method Failed', {
			error: error.message
		}, 'warn');
	}
 ...
 */
function buildDivisionsAndManagersEnhanced({previousRosters, leagueMetadata, numDivisions, playoffParticipants}) {
	const divisions = {};
	...
	for(const rosterID in previousRosters) {
		...
		if(rSettings.wins > divisions[div].wins || 
		   (rSettings.wins == divisions[div].wins && totalPoints > divisions[div].points)) {
			...
			divisions[div].isPlayoffTeam = playoffParticipants ? 
				playoffParticipants.has(parseInt(rosterID)) : false;
		}
	}
 ...
 */
function calculatePodiumConfidence(championshipInfo, toiletInfo, playoffStructure) {
	let confidence = 100;
