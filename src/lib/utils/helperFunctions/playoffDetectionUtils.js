...
export function parsePlayoffBrackets(winnersData, losersData, leagueSettings) {
  ...
  const playoffFormat = determinePlayoffFormat(
    winnersData,
    losersData,
    leagueSettings,
  );
  ...
  const playoffStructure = {
    ...
    validationResults: validatePlayoffStructure(
      playoffParticipants,
      championshipInfo,
      playoffFormat,
    ),
...
function determinePlayoffFormat(winnersData, losersData, leagueSettings) {
  ...
  if (!playoffTeams) {
    ...
    winnersData.forEach((match) => {
      if (match.t1 && typeof match.t1 === "number")
        teamsInBracket.add(match.t1);
      if (match.t2 && typeof match.t2 === "number")
        teamsInBracket.add(match.t2);
    });
 ...
 */
function identifyPlayoffParticipants(
  winnersData,
  losersData,
  bracketStructure,
  playoffFormat,
  ...
  const firstRoundParticipants =
    extractFirstRoundParticipants(bracketStructure);
  ...
  const bracketTraversalParticipants = traverseBracketForParticipants(
    winnersData,
    losersData,
  );
  ...
  if (actualCount !== expectedCount) {
    debugAwardProcessing(
      "Playoff Count Mismatch - Additional Validation",
      {
    ...
    if (losersData && Array.isArray(losersData) && losersData.length > 0) {
      const losersParticipants =
        extractParticipantsFromLosersBracket(losersData);
    ...
    if (parseInt(round) > 1) {
      matches.forEach((match) => {
        ...
        if (
          ...
          !firstRoundTeams.has(match.team1) &&
          !match.team1From
        ) {
          participants.add(match.team1);
        }
        if (
          ...
          !firstRoundTeams.has(match.team2) &&
          !match.team2From
        ) {
          participants.add(match.team2);
        }
...
function extractChampionshipInfo(winnersData, playoffFormat) {
  ...
  const championshipMatch = winnersData.find(
    (m) => m.r === finalRound && m.w && m.l,
  );
  ...
  const thirdPlaceMatch = winnersData.find(
    (m) => m.r === finalRound - 1 && m.t1_from?.l && m.t2_from?.l, // Match between losers
  );
 ...
 */
function validatePlayoffStructure(
  participants,
  championshipInfo,
  playoffFormat,
) {
  ...
  if (!championshipInfo.championshipMatch) {
    ...
    if (champion && !participants.has(champion)) {
      validation.issues.push(
        `Champion (${champion}) not found in participant list`,
      );
      validation.isValid = false;
    }
    ...
    if (runnerUp && !participants.has(runnerUp)) {
      validation.issues.push(
        `Runner-up (${runnerUp}) not found in participant list`,
      );
      validation.isValid = false;
    }
  }
  ...
  if (playoffFormat.hasByes && participants.size % 2 === 0) {
    validation.warnings.push(
      "Format indicates byes but participant count is even",
    );
  }
...
function createEmptyPlayoffStructure() {
  return {
    ...
    validationResults: {
      isValid: false,
      issues: ["No bracket data available"],
      warnings: [],
    },
  };
}
 ...
 */
export function analyzePlayoffParticipation(
  rosterID,
  year,
  winnersData,
  losersData,
  ...
  const analysis = {
    madePlayoffs,
    reason: madePlayoffs
      ? "Found in playoff bracket"
      : "Not found in playoff participants",
    confidence,
    details: {
...
function analyzePlayoffAchievements(rosterID, playoffStructure) {
  ...
  let lastRound = 0;
  Object.entries(playoffStructure.bracketStructure.rounds).forEach(
    ([round, matches]) => {
      const appearsInRound = matches.some(
        (match) =>
          match.team1 === rosterID ||
          match.team2 === rosterID ||
          match.winner === rosterID ||
          match.loser === rosterID,
      );
      if (appearsInRound) {
