 ...
 */
export function validateManagerStatistics(
  managerStats,
  managerID = null,
  ...
  if ("seasons" in managerStats) {
    const seasonsValidation = validateSeasonsArray(
  ...
  if ("totalStats" in managerStats) {
    const totalStatsValidation = validateManagerTotalStats(
  ...
  const advancedValidation = validateAdvancedManagerMetrics(
    managerStats,
    context,
  );
...
export function validateSeasonsArray(seasons, context = "seasons") {
  ...
  seasons.forEach((season, index) => {
    const seasonValidation = validateEnhancedSeasonData(
      season,
      `${context} season ${index}`,
    );
...
export function validateEnhancedSeasonData(seasonData, context = "season") {
  ...
  if ("playoffs" in seasonData && typeof seasonData.playoffs === "boolean") {
    ...
    if (seasonData.playoffs) {
      ...
      if (winRate < 0.4) {
        // Less than 40% win rate but made playoffs
        result.addWarning(
          VALIDATION_ERRORS.INCONSISTENT_DATA,
  ...
  if ("validationMetadata" in seasonData) {
    const metadataValidation = validateValidationMetadata(
      seasonData.validationMetadata,
      context,
    );
 ...
 */
export function validateValidationMetadata(
  metadata,
  ...
  if (
    "detectionMethods" in metadata &&
    Array.isArray(metadata.detectionMethods)
  ) {
...
export function validateManagerTotalStats(totalStats, context = "total stats") {
  ...
  if (
    "winPercentage" in totalStats &&
    typeof totalStats.winPercentage === "number"
  ) {
  ...
  if (
    "averagePointsPerGame" in totalStats &&
    typeof totalStats.averagePointsPerGame === "number"
  ) {
  ...
  if (totalStats.seasonsPlayed > 0) {
    const playoffRate =
      (totalStats.playoffAppearances / totalStats.seasonsPlayed) * 100;
    const championshipRate =
      (totalStats.championships / totalStats.seasonsPlayed) * 100;
 ...
 */
export function validateSeasonsToTotalsConsistency(
  seasons,
  totalStats,
  ...
  const calculatedTotals = {
    ...
    totalPointsAgainst: seasons.reduce(
      (sum, s) => sum + (s.fptsAgainst || 0),
      0,
    ),
  ...
  const totalGames =
    calculatedTotals.totalWins +
    calculatedTotals.totalLosses +
    calculatedTotals.totalTies;
 ...
 */
export function validateAdvancedManagerMetrics(
  managerStats,
  ...
  const perfectSeasons = seasons.filter(
    (s) => (s.losses || 0) === 0 && (s.wins || 0) > 0,
  );
  if (
    perfectSeasons.length >
    MANAGER_VALIDATION_CONFIG.WARNING_THRESHOLDS.PERFECT_SEASON_COUNT
  ) {
    result.addWarning(
      VALIDATION_ERRORS.OUT_OF_RANGE,
  ...
  if (efficiencies.length > 0) {
    ...
    const perfectEfficiencySeasons = efficiencies.filter(
      (eff) =>
        eff >=
        MANAGER_VALIDATION_CONFIG.WARNING_THRESHOLDS.EFFICIENCY_THRESHOLD,
    );
 ...
 */
export function validateHeadToHeadRecords(
  h2hRecords,
  context = "head-to-head records",
) {
  const result = new ValidationResult();
  ...
  Object.entries(h2hRecords).forEach(([opponentId, record]) => {
    ...
    ["wins", "losses", "ties", "pointsFor", "pointsAgainst"].forEach(
      (field) => {
    ...
    if (calculatedTotal > 30) {
      // More than 30 games vs one opponent seems high
      result.addWarning(
        VALIDATION_ERRORS.OUT_OF_RANGE,
 ...
 */
export function validateManagerStatsEnhanced(
  managerStats,
  managerID = null,
  options = {},
) {
  ...
  if (includeAdvancedMetrics && result.isValid) {
    const advancedResult = validateAdvancedManagerMetrics(
      managerStats,
      `${context} Advanced`,
    );
