...
async function fetchWithValidation(url, options = {}) {
    ...
    for (let attempt = 1; attempt <= API_CONFIG.RETRY_ATTEMPTS; attempt++) {
        try {
            console.log(`[API_FETCH] Attempt ${attempt}/${API_CONFIG.RETRY_ATTEMPTS}: ${url}`);
            ...
            if (!responseValidation.isValid) {
                ...
                if (response.status >= 500 && attempt < API_CONFIG.RETRY_ATTEMPTS) {
                    console.warn(`[API_FETCH] Server error ${response.status}, retrying...`);
                    await new Promise(resolve => setTimeout(resolve, API_CONFIG.RETRY_DELAY * attempt));
                    continue;
                }
                
                throw new Error(`API request failed: ${responseValidation.errors.map(e => e.message).join(', ')}`);
            }
            ...
            if (attempt < API_CONFIG.RETRY_ATTEMPTS) {
                console.warn(`[API_FETCH] Attempt ${attempt} failed, retrying:`, error.message);
                await new Promise(resolve => setTimeout(resolve, API_CONFIG.RETRY_DELAY * attempt));
            } else {
    ...
    clearTimeout(timeoutId);
    throw lastError || new Error(`Failed to fetch ${url} after ${API_CONFIG.RETRY_ATTEMPTS} attempts`);
}
...
async function parseAndValidateJson(response, url, validator = null) {
    try {
        ...
        if (!jsonValidation.isValid) {
            ...
            throw new Error(`Invalid JSON structure: ${jsonValidation.errors.map(e => e.message).join(', ')}`);
        }
...
export const getLeagueDataValidated = async (queryLeagueID = leagueID) => {
    ...
    if (get(leagueData)[queryLeagueID]) {
        ...
        logValidationResult(cacheValidation, `Cached League Data: ${queryLeagueID}`);
        
        if (cacheValidation.isValid) {
            return existingData;
        } else {
            console.warn(`[CACHE_VALIDATION] Cached league data is invalid, refetching...`);
        }
    }
    ...
    try {
        ...
        console.log(`[LEAGUE_DATA] Successfully fetched and validated league data for ${queryLeagueID}`);
        return data;
        
    } catch (error) {
        console.error(`[LEAGUE_DATA] Failed to fetch league data for ${queryLeagueID}:`, error);
...
export const getRostersValidated = async (queryLeagueID = leagueID) => {
    ...
    try {
        ...
        console.log(`[ROSTERS] Successfully fetched and validated ${data.length} rosters for league ${queryLeagueID}`);
        return data;
        
    } catch (error) {
        console.error(`[ROSTERS] Failed to fetch roster data for ${queryLeagueID}:`, error);
 ...
 */
export const getMatchupsValidated = async (queryLeagueID = leagueID, week = 1) => {
    const url = `${API_CONFIG.BASE_URL}/league/${queryLeagueID}/matchups/${week}`;
    
    try {
        const response = await fetchWithValidation(url);
        const data = await parseAndValidateJson(response, url, (data) => validateMatchupData(data, week));
        
        console.log(`[MATCHUPS] Successfully fetched and validated week ${week} matchups for league ${queryLeagueID}`);
        return data;
        
    } catch (error) {
        console.error(`[MATCHUPS] Failed to fetch matchup data for week ${week}, league ${queryLeagueID}:`, error);
 ...
 */
export const getBracketsValidated = async (queryLeagueID = leagueID, bracketType = 'winners_bracket') => {
    const url = `${API_CONFIG.BASE_URL}/league/${queryLeagueID}/${bracketType}`;
    
    try {
        const response = await fetchWithValidation(url);
        const data = await parseAndValidateJson(response, url, (data) => validateBracketData(data, bracketType));
        
        console.log(`[BRACKETS] Successfully fetched and validated ${bracketType} for league ${queryLeagueID}`);
        return data;
        
    } catch (error) {
        console.error(`[BRACKETS] Failed to fetch ${bracketType} for league ${queryLeagueID}:`, error);
...
export const getUsersValidated = async (queryLeagueID = leagueID) => {
    ...
    try {
        ...
        console.log(`[USERS] Successfully fetched and validated ${data.length} users for league ${queryLeagueID}`);
        return data;
        
    } catch (error) {
        console.error(`[USERS] Failed to fetch user data for ${queryLeagueID}:`, error);
 ...
 */
export const getMultipleMatchupsValidated = async (queryLeagueID = leagueID, weeks = []) => {
    ...
    if (errors.length > 0) {
        console.warn(`[MULTI_MATCHUPS] ${errors.length} weeks failed to fetch:`, errors);
    }
    
    console.log(`[MULTI_MATCHUPS] Successfully fetched ${Object.keys(results).length}/${weeks.length} weeks`);
 ...
 */
export const getLeagueDataWithConsistencyCheck = async (queryLeagueID = leagueID) => {
    try {
        ...
        console.error(`[CONSISTENCY_CHECK] Failed for league ${queryLeagueID}:`, error);
        throw error;
    }
};
 ...
 */
export const getHistoricalLeagueDataValidated = async (startLeagueID = leagueID, maxSeasons = 10) => {
    const historicalData = {};
    const validationResults = {};
    let currentLeagueID = startLeagueID;
    let seasonsProcessed = 0;
    
    while (currentLeagueID && currentLeagueID !== '0' && seasonsProcessed < maxSeasons) {
        try {
            console.log(`[HISTORICAL] Processing season ${seasonsProcessed + 1}, league ${currentLeagueID}`);
            ...
            console.error(`[HISTORICAL] Failed to process league ${currentLeagueID}:`, error);
    ...
    const summary = {
        ...
        years: Object.keys(historicalData).map(Number).sort((a, b) => b - a),
        validationResults
    };
 ...
 */
export const getDataWithFallback = async (fetchFunction, cacheKey, fallbackData = null) => {
    try {
        return await fetchFunction();
    } catch (error) {
        console.warn(`[FALLBACK] Primary fetch failed, attempting fallback:`, error.message);
...
export const performApiHealthCheck = async () => {
    ...
    const testEndpoints = [
        { name: 'league', url: `${API_CONFIG.BASE_URL}/league/${leagueID}`, validator: validateLeagueData },
        { name: 'rosters', url: `${API_CONFIG.BASE_URL}/league/${leagueID}/rosters`, validator: validateRosterData },
        { name: 'nfl_state', url: `${API_CONFIG.BASE_URL}/state/nfl`, validator: null }
    ];
    ...
    for (const endpoint of testEndpoints) {
        try {
            ...
            const data = await parseAndValidateJson(response, endpoint.url, endpoint.validator);
    ...
    healthCheck.overallStatus = passedTests === testEndpoints.length ? 'healthy' : 
