 ...
 */
export async function runTRLComprehensiveTest(
  testMode = TEST_CONFIG.QUICK_TEST,
) {
  ...
  try {
    ...
    if (
      testMode === TEST_CONFIG.PERFORMANCE_TEST ||
      testMode === TEST_CONFIG.FULL_TEST
    ) {
...
async function testBasicValidationFunctions() {
  ...
  return {
    ...
    failureRate:
      summary.metrics.validationsFailed / summary.metrics.validationsRun,
  };
}
...
async function testDataConsistencyChecks() {
  try {
    ...
    return {
      ...
      consistencyWarnings:
        consistencyResult.consistencyCheck.warnings?.length || 0,
    };
  } catch (error) {
    return {
      success: false,
...
async function testValidationErrorHandling() {
  ...
  try {
    ...
    results.invalidLeagueID = {
      success: false,
      message: "Should have failed with invalid ID",
    };
  } catch (error) {
    results.invalidLeagueID = {
      success: true,
...
async function testManagerStatistics(testMode) {
  ...
  for (let i = 0; i < managers.length; i++) {
    ...
    if (testMode === TEST_CONFIG.QUICK_TEST && i >= 3) {
      console.log(
        `  ⏩ Quick mode: skipping remaining ${managers.length - i} managers`,
      );
      break;
    }
    ...
    try {
      console.log(
        `    Processing manager ${i + 1}/${managers.length}: ${manager.name}`,
      );
      ...
      if (managerResult.success) {
        ...
        console.warn(
          `    ⚠️ Manager ${manager.name} failed validation:`,
          managerResult.errors,
        );
      }
      ...
      if (
        managerResult.suspiciousZeros &&
        managerResult.suspiciousZeros.length > 0
      ) {
...
function createMockManagerStats(manager) {
  ...
  const seasonsPlayed = Math.min(
    5,
    currentYear - (manager.fantasyStart || 2020) + 1,
  );
  ...
  totalStats.winPercentage =
    totalGames > 0 ? (totalStats.totalWins / totalGames) * 100 : 0;
  totalStats.averagePointsPerSeason =
    seasonsPlayed > 0 ? totalStats.totalPoints / seasonsPlayed : 0;
  totalStats.averagePointsPerGame =
    totalGames > 0 ? totalStats.totalPoints / totalGames : 0;
...
function generateTestSummary(testResults) {
  ...
  if (testResults.results.managerStats) {
    ...
    if (managerStats.nanValuesDetected?.length > 0) {
      summary.criticalIssues.push(
        `NaN values detected in ${managerStats.nanValuesDetected.length} managers`,
      );
      summary.overallSuccess = false;
    }
    ...
    if (managerStats.zeroCountsDetected?.length > 0) {
      ...
      summary.recommendations.push(
        "Review suspicious zero counts in manager statistics",
      );
    }
  }
  ...
  if (testResults.results.performance) {
    ...
    Object.entries(performance.apiResponseTimes || {}).forEach(
      ([api, result]) => {
        if (
          result.responseTime > TEST_CONFIG.THRESHOLDS.MAX_API_RESPONSE_TIME
        ) {
          summary.recommendations.push(
            `${api} API response time is slow: ${result.responseTime}ms`,
          );
        }
      },
    );
...
function displayTestResults(testResults) {
  ...
  if (testResults.results.managerStats) {
    ...
    if (ms.nanValuesDetected?.length > 0) {
      console.log(
        `  🚨 NaN values detected: ${ms.nanValuesDetected.length} managers`,
      );
    }

    if (ms.zeroCountsDetected?.length > 0) {
      console.log(
        `  ⚠️  Suspicious zeros: ${ms.zeroCountsDetected.length} managers`,
      );
    }
  }
...
export async function quickValidationTest() {
  ...
  try {
    ...
    console.log(
      `📊 League: ${leagueData.name}, Season: ${leagueData.season}, Rosters: ${leagueData.total_rosters}`,
    );

    // Test validation system
    const testSuite = await runValidationTestSuite(leagueID, {
      scenario:
        VALIDATION_INTEGRATION_CONFIG.TEST_SCENARIOS.BASIC_FUNCTIONALITY,
      generateReport: false,
    });

    console.log("✅ Validation test suite completed");
    console.log(
      `📈 Success rate: ${testSuite.summary.passedTests}/${testSuite.summary.totalTests}`,
    );
...
export async function testSpecificManager(managerIndex = 0) {
  if (managerIndex >= managers.length) {
    console.error(
      `❌ Manager index ${managerIndex} out of range (0-${managers.length - 1})`,
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `📊 Errors: ${validation.errors?.length || 0}, Warnings: ${validation.warnings?.length || 0}`,
    );
