 ...
 */
export const extractUniqueTeamsFromTransactions = (
  transactions,
  leagueTeamManagers,
) => {
  ...
  try {
    ...
    if (!Array.isArray(transactions)) {
      console.warn(
        "teamFilterUtils: transactions is not an array, received:",
        typeof transactions,
      );
      return result;
    }
    ...
    if (!leagueTeamManagers || typeof leagueTeamManagers !== "object") {
      console.warn(
        "teamFilterUtils: leagueTeamManagers is invalid, received:",
        typeof leagueTeamManagers,
      );
      return result;
    }
    ...
    if (
      ...
      typeof leagueTeamManagers.teamManagersMap !== "object"
    ) {
      console.warn(
        "teamFilterUtils: leagueTeamManagers.teamManagersMap is missing",
      );
      return result;
    }
    ...
    transactions.forEach((transaction, index) => {
      try {
        ...
        if (!transaction || typeof transaction !== "object") {
          console.warn(
            `teamFilterUtils: Invalid transaction at index ${index}`,
          );
        ...
        if (!Array.isArray(rosters)) {
          ...
          console.warn(
            `teamFilterUtils: Transaction ${transaction.id || index} has invalid rosters:`,
            rosters,
          );
          return; // Continue to next transaction
        }
        ...
        rosters.forEach((rosterID) => {
          try {
            ...
            if (typeof rosterID !== "number" && typeof rosterID !== "string") {
              console.warn(
                `teamFilterUtils: Invalid roster ID in transaction ${transaction.id || index}:`,
                rosterID,
              );
            ...
            if (isNaN(numericRosterID)) {
              console.warn(
                `teamFilterUtils: Could not parse roster ID:`,
                rosterID,
              );
              return;
            }
            ...
            if (!result.teamSeasonHistory.has(teamKey)) {
              ...
              const teamData =
                leagueTeamManagers.teamManagersMap?.[season]?.[numericRosterID];
            ...
            console.error(
              `teamFilterUtils: Error processing roster ${rosterID} in transaction ${transaction.id || index}:`,
              rosterError,
            );
        ...
        console.error(
          `teamFilterUtils: Error processing transaction at index ${index}:`,
          transactionError,
        );
    ...
    if (result.uniqueRosterIDs.size > 0) {
      console.log(
        `teamFilterUtils: Successfully extracted ${result.uniqueRosterIDs.size} unique teams from ${transactions.length} transactions`,
      );
    }
  } catch (error) {
    console.error(
      "teamFilterUtils: Critical error in extractUniqueTeamsFromTransactions:",
      error,
    );
  ...
  try {
    ...
    if (!(uniqueRosterIDs instanceof Set)) {
      ...
      validation.warnings.push(
        "No unique roster IDs found - may indicate empty transaction history",
      );
    }
    ...
    if (uniqueRosterIDs instanceof Set && teamSeasonHistory instanceof Map) {
      ...
      if (rostersInHistory.size !== uniqueRosterIDs.size) {
        validation.warnings.push(
          `Roster count mismatch: ${uniqueRosterIDs.size} unique vs ${rostersInHistory.size} in history`,
        );
      }
    }
 ...
 */
export const createTeamLookupMap = (
  uniqueRosterIDs,
  leagueTeamManagers,
  currentSeason,
) => {
  ...
  try {
      ...
      try {
        ...
        if (!teamData) {
          ...
          for (const fallbackSeason of seasonsToCheck) {
            teamData =
              leagueTeamManagers.teamManagersMap?.[fallbackSeason]?.[rosterID];
            if (teamData) {
        ...
        console.error(
          `teamFilterUtils: Error processing roster ${rosterID} in createTeamLookupMap:`,
          error,
        );
        // Add fallback entry
        teamLookupMap.set(rosterID, {
    ...
    console.log(
      `teamFilterUtils: Created lookup map for ${teamLookupMap.size} teams`,
    );
  } catch (error) {
    console.error(
      "teamFilterUtils: Critical error in createTeamLookupMap:",
      error,
    );
  }
...
export const getTeamDataForSeason = (rosterID, season, leagueTeamManagers) => {
  try {
    ...
    if (!leagueTeamManagers?.teamManagersMap) {
      console.warn(
        "teamFilterUtils: Invalid leagueTeamManagers in getTeamDataForSeason",
      );
      return null;
    }
    ...
    if (isNaN(numericRosterID) || isNaN(numericSeason)) {
      console.warn(
        "teamFilterUtils: Invalid rosterID or season in getTeamDataForSeason",
      );
      return null;
    }

    const teamData =
      leagueTeamManagers.teamManagersMap[numericSeason]?.[numericRosterID];
...
export const generateTeamFilterOptions = (teamLookupMap) => {
  try {
    if (
      !teamLookupMap ||
      !(teamLookupMap instanceof Map) ||
      teamLookupMap.size === 0
    ) {
      console.warn(
        "teamFilterUtils: Invalid or empty teamLookupMap in generateTeamFilterOptions",
      );
      return [
        {
          value: null,
    ...
    console.error(
      "teamFilterUtils: Error in generateTeamFilterOptions:",
      error,
    );
    return [
      {
...
export const isTeamInvolvedInTransaction = (transaction, targetRosterID) => {
  try {
    ...
    console.error(
      "teamFilterUtils: Error in isTeamInvolvedInTransaction:",
      error,
    );
    return false;
  }
 ...
 */
export const filterTransactionsByTeamInvolvement = (
  transactions,
  targetRosterID,
  teamLookupMap = null,
) => {
  try {
    ...
    if (!Array.isArray(transactions)) {
      console.warn(
        "teamFilterUtils: transactions is not an array in filterTransactionsByTeamInvolvement",
      );
      return [];
    }
    ...
    if (isNaN(numericTargetID)) {
      console.warn(
        "teamFilterUtils: Invalid targetRosterID in filterTransactionsByTeamInvolvement",
      );
      return [];
    }
    ...
    console.error(
      "teamFilterUtils: Error in filterTransactionsByTeamInvolvement:",
      error,
    );
    return [];
  }
