...
function processSeasonData(yearData, awards, managerRosters) {
  ...
  const processedSeason = {
    ...
    lineupEfficiency:
      potentialPoints > 0
        ? parseFloat(round((fpts / potentialPoints) * 100))
        : 0,
    rosterID: yearData.rosterID,
 ...
 */
function computeManagerStatsLegacy(
  manager,
  leagueTeamManagers,
  records,
  currentRosterID,
  ...
  for (const yearData of seasonData) {
    const season = processSeasonData(
      { ...yearData, rosterID: currentRosterID },
      awards,
      {},
    );
    seasons.push(season);
...
export function computeManagerStats(
  ...
  if (!manager?.managerID) {
    return computeManagerStatsLegacy(
      manager,
      leagueTeamManagers,
      records,
      currentRosterID,
  ...
  const managerRosters = getAllManagerRosterIDs(
    manager.managerID,
    leagueTeamManagers,
  );
  ...
  for (const yearData of seasonData) {
    ...
    totalStats.totalPointsAgainst += isNaN(season.fptsAgainst)
      ? 0
      : season.fptsAgainst;
    totalStats.seasonsPlayed++;
  ...
  while (curSeason && curSeason != 0) {
    try {
      ...
      const matchupsData = await waitForAll(
        ...matchupsResults.map((res) => res.json()),
      );
      ...
      for (let weekIndex = 0; weekIndex < matchupsData.length; weekIndex++) {
        ...
        const managerMatchup = weekMatchups.find(
          (m) => m.roster_id == managerRosterID,
        );
        ...
        if (opponentMatchup) {
          historicalMatchups.push({
            ...
            tied:
              (managerMatchup.points || 0) === (opponentMatchup.points || 0),
          });
        }
      }
 ...
 */
function processHeadToHeadRecords(
  historicalMatchups,
  ...
  for (const [opponentRosterID, matchups] of Object.entries(opponentGroups)) {
    ...
    for (const [year, yearData] of Object.entries(
      ...
      if (
        yearData[opponentRosterID] &&
        yearData[opponentRosterID].managers.length > 0
      ) {
  ...
  for (const [opponentManagerID, record] of Object.entries(h2hRecords)) {
    ...
    const currentYear = Math.max(
      ...Object.keys(leagueTeamManagers.teamManagersMap || {}).map(Number),
    );
    ...
    if (yearData) {
      for (const [rosterID, rosterData] of Object.entries(yearData)) {
        if (
          rosterData.managers &&
          rosterData.managers.includes(opponentManagerID)
        ) {
...
export async function computeHeadToHeadRecords(
  ...
  try {
    ...
    const { historicalMatchups, managerRosterHistory } =
      await getHistoricalMatchupData(manager.managerID, leagueTeamManagers);
...
export function computeManagerRankings(records, rosterID) {
  ...
  const allRecords = Object.entries(
  ...
  const efficiencyRanking =
    allRecords
      .sort(
        (a, b) =>
          b.fptsFor / (b.potentialPoints || 1) -
          a.fptsFor / (a.potentialPoints || 1),
      )
      .findIndex((record) => record.rosterID === rosterID) + 1;
...
function crossValidateAwardsAndBrackets(awardResults, playoffAnalysis) {
  ...
  if (playoffAnalysis?.confidence) {
    ...
    crossValidated.confidence = Math.min(
      100,
      crossValidated.confidence * (0.7 + 0.3 * bracketConfidenceFactor),
    );
  }
...
function processSeasonFields(yearData) {
  ...
  const lineupEfficiency =
    potentialPoints > 0 ? parseFloat(round((fpts / potentialPoints) * 100)) : 0;
...
async function processSeasonDataEnhanced(
  ...
  const crossValidatedResults = crossValidateAwardsAndBrackets(
    awardResults,
    playoffAnalysis,
  );
...
export async function computeManagerStatsEnhanced(
  ...
  const standardStats = computeManagerStats(
    manager,
    leagueTeamManagers,
    records,
    currentRosterID,
  ...
  const totalGames =
    enhancedTotalStats.totalWins +
    enhancedTotalStats.totalLosses +
    enhancedTotalStats.totalTies;
  enhancedTotalStats.winPercentage =
    totalGames > 0
      ? parseFloat(round((enhancedTotalStats.totalWins / totalGames) * 100))
      : 0;
  enhancedTotalStats.averagePointsPerSeason =
    enhancedTotalStats.seasonsPlayed > 0
      ? parseFloat(
          round(
            enhancedTotalStats.totalPoints / enhancedTotalStats.seasonsPlayed,
          ),
        )
      : 0;
  enhancedTotalStats.averagePointsPerGame =
    totalGames > 0
      ? parseFloat(round(enhancedTotalStats.totalPoints / totalGames))
      : 0;
  ...
  const enhancedStats = {
    ...
    enhancementMetadata: {
      ...
      seasonsEnhanced: enhancedSeasons.filter(
        (s) => s.validationMetadata?.playoffAnalysis,
      ).length,
      averageConfidence:
        enhancedSeasons
          .filter((s) => s.validationMetadata?.crossValidationConfidence)
          .reduce(
            (sum, s) => sum + s.validationMetadata.crossValidationConfidence,
            0,
          ) / enhancedSeasons.length,
    },
  };
