<script>
    ...
    // Dropdown state for record selection
    let selectedRecordType = 'weekRecords';
    let selectedSpecificTable = 'weekHighs';

    // Record type configuration for dropdown interface
    const recordTypes = {
        weekRecords: {
            label: 'Week Records',
            tables: {
    ...
    // Reactive statement to reset specific table when record type changes
    $: if (selectedRecordType && recordTypes[selectedRecordType]) {
        const availableTables = Object.keys(recordTypes[selectedRecordType].tables);
        if (availableTables.length > 0) {
            selectedSpecificTable = availableTables[0];
        }
    }
...
<style>
    ...
    .dropdown-row :global(.mdc-select) {
        min-width: 200px;
    }
    ...
    .single-table-display :global(.recordTable) {
        margin: 0 auto;
        width: 100%;
    }
