...
async function investigateLeague(leagueID, year) {
    try {
        ...
        if (!leagueResponse.ok) {
            console.error(`Failed to fetch league data: ${leagueResponse.status} ${leagueResponse.statusText}`);
            return;
        }
        ...
        if (users.length > 0) {
            ...
            users.forEach((user, index) => {
                console.log(`  ${index + 1}. ${user.display_name} (ID: ${user.user_id}) - Owner: ${user.is_owner || false}`);
        ...
        if (rosters.length > 0) {
            ...
            rosters.forEach((roster, index) => {
                console.log(`  Roster ${roster.roster_id}: Owner ID ${roster.owner_id || 'NULL'}`);
            });
        }
